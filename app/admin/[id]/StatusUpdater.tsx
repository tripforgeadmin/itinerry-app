...
export default function StatusUpdater({ id, currentStatus, inline = false }: { id: string; currentStatus: string; inline?: boolean }) {
  ...
  // header variant — no card wrapper, everything on one wrapping row
  if (inline) {
    return (
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm p-4 mb-4">
      <div className="flex items-center gap-3">
        ...
        <span className={`px-2.5 py-1 rounded-lg text-xs font-bold ${currentColor}`}>{currentLabel}</span>
        <div className="flex gap-2 ml-auto flex-wrap">{statusButtons}</div>
      </div>
      {hasPendingChange && (

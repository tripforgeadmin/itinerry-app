...
const KIND_BADGE: Record<string, { label: string; cls: string }> = {
  ticket_received: { label: "รับเรื่อง", cls: "bg-blue-100 text-blue-700" },
  follow_up: { label: "ติดตาม", cls: "bg-blue-100 text-blue-700" },
  share_card: { label: "การ์ดแชร์", cls: "bg-purple-100 text-purple-700" },
  result: { label: "ส่งผล", cls: "bg-green-100 text-green-700" },
  manual: { label: "พิมพ์เอง", cls: "bg-gray-100 text-gray-600" },
};
...
export default function MessageLogPanel({ assessmentId }: { assessmentId: string }) {
  ...
  return (
    <div className="flex h-full flex-col rounded-2xl bg-white shadow-sm">
      <div className="flex items-center gap-2 border-b border-gray-100 px-4 py-3">
        <span className="text-xs font-bold uppercase tracking-wider text-gray-400">ข้อความถึงลูกค้า</span>
        <span className="ml-auto text-[11px] text-gray-300">{messages.length} รายการ</span>
      </div>

      <div ref={listRef} className="flex-1 space-y-2 overflow-y-auto px-3 py-3">
        {!loaded ? (
          <p className="pt-8 text-center text-xs text-gray-300">กำลังโหลด…</p>
        ) : messages.length === 0 ? (
          <p className="pt-8 text-center text-xs text-gray-300">ยังไม่เคยส่งข้อความถึงลูกค้าคนนี้</p>
        ) : (
          messages.map((m, i) => {
            const badge = KIND_BADGE[m.kind];
            const newDay = i === 0 || dayLabel(messages[i - 1].created_at) !== dayLabel(m.created_at);
            ...
            return (
              <div key={m.id}>
                {newDay && (
                  <div className="my-2 text-center text-[10px] text-gray-300">{dayLabel(m.created_at)}</div>
                )}
                <div className="flex flex-col items-end">
                  <div className="mb-0.5 flex items-center gap-1 text-[10px] text-gray-400">
                    {!m.delivered && <span className="font-bold text-red-500">ส่งไม่สำเร็จ ·</span>}
                    <span>{m.sent_by === "admin" ? "แอดมิน" : "ระบบ"} · {timeLabel(m.created_at)}</span>
      ...
      <div className="border-t border-gray-100 px-3 py-3">
        {loaded && !canSend && (
          <p className="mb-2 text-[11px] text-amber-600">
            {isFriend === false
              ? "ลูกค้ายังไม่ได้เพิ่มเพื่อน LINE OA — ส่งข้อความไม่ได้"
              : "ลูกค้าไม่มีบัญชี LINE ในระบบ — ส่งข้อความไม่ได้"}
          </p>
        )}
        <textarea
          value={text}

...
export default async function AdminPage() {
  const [{ data, error }, { data: slaCfg }] = await Promise.all([
    supabase
      ...
      .select(
        "id, ticket_id, created_at, due_date, status, contact_preference, intent, savings_balance, result_sent_at, status_history(changed_at), account:account_id(nickname, full_name, first_name, last_name, line_display_name, phone, phone_country_code, is_friend), trip:trip_id(visa_type, destination, travel_arrival, study_start), visa_evaluation(pass, strengths, improvements)"
      )

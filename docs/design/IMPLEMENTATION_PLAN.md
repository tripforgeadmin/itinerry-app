...
>
> **Group C progress:** ✅ C1 อาชีพ (occupation q24 bespoke alternating-image + empdoc q25–q28 SegmentedScreen + q29 ChoiceScreen). Next C2 คุณสมบัติ (refused/overstay/savings/ties — needs obligation mascots). Then C3 ข้อมูลติดต่อ — **blocked on 3 decisions:** (1) contact-field ordering (name/phone/email early in as-is vs design's merged-late contact screen), (2) found q7 single-radio vs design multi-select, (3) whether to add the design's bare `transition` interstitial (not in as-is data). Phase-transition elephant loaders (visatype→dates, occupation→empdoc, summary→submit) still deferred to Phase 4.
>

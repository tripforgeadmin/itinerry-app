...
>
> **Group C progress:** ✅ C1 อาชีพ (q24–q29) · ✅ C2 คุณสมบัติ (q30–q35) · ✅ C3a flow re-sequence + phase-transition loaders. **Decisions taken:** re-sequence contact fields to the end (design order); found q7 stays single-radio; the "transition" is the **elephant-iPad loader** (not a routed screen). Flow graph rewired in `lib/questions.ts` (FIRST = q4; q4→q8→q9→…→q35→q3→q5→q6→q36→q37→q7→q2 terminal) — verified by trace (36 reachable, no dangling/orphan). `ElephantLoader` now uses `itin-hold-ipad.png` (resized 480px, 296K) and fires on advancing OUT of the 5-category boundaries — q9 (พื้นฐาน→เดินทาง), the work-branch ends q25/q27/q28/q29 (อาชีพ→คุณสมบัติ), q35 (คุณสมบัติ→ข้อมูลติดต่อ); regular per-step fade transitions unchanged. **Next C3b:** merged ContactScreen (q3+q5+q6+q36+q37 via advanceTo), FoundScreen (q7 single, logo tiles), SummaryScreen (q2 = review card + certify).
>
